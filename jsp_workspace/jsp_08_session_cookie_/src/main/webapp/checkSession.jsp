...
<body>
	...
	<%
		// tomcat server 설정 기본 session 유지 시간 : 1800 - 30분
		// project.web xml에 설정된 session 유지시간 : 600s - 10분
		
		// 해당 session만 유지 시간을 60s로 지정
		...
		// session 객체가 생성된 시간을  millis로 반환
		long create = session.getCreationTime();
		String time = sdf.format(create);
		out.println("현재 session 생성 시간 : " + create + "<br/>");
