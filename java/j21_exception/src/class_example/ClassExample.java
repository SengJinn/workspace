	...
	public static void main(String[] args) {
		...
		try {
			...
			for(Field f : fields) {
				...
				System.out.println(f.getType().getSimpleName()+"\t"); // Field type
			...
			for(Constructor<?> c : cons) {
				...
				for(int i = 0; i < type.length; i ++) {
					System.out.println(type[i].getName()+" " + names[i].getName()+"\t");
				}
				System.out.println(")");
			}
			...
			// 객체 설계정보는 해당 위치에 존재하지만 객체 생성이 불가능할 때 발생하는 오류
			// 추상 클래스, interface 이러한 설계 정보를 가진 class 파일은
			// 자체만으로 객체 생성이 불가하니 생성할 수 없다고 발생하는 예외
		} catch (IllegalAccessException e) {
			// class 정보도 존재하고 일반 class이지만
			...
			// 오버로딩된 생성자를 찾지 못 함
			// 동일한 개수와 매개변수 타입을 가진 생성자를 찾지 못 함
		} catch (SecurityException e) {
			// 접근권한이 없을 때 - 파일이 접근할 수 없는 디렉토리에 존재할 때
		}
