...
void *thread_func(void *targp) { // maybe need lock
	...
	for (a = 0; a < 4; a++) {
		bzero(net_output, 3);
		strncpy(&net_output[0], &buffer_[a*3+2], sizeof(char));	// there were
		strncpy(&net_output[1], &buffer_[a*3+3], sizeof(char));	// some errors
		strncpy(&net_output[2], &buffer_[a*3+4], sizeof(char));	// in strncpy
...
int main(int argc, char **argv)
{
	...
	timer = al_create_timer(0.2); // time checking
	...
	while(running) {
		...
		if (!al_is_event_queue_empty(event_queue)) {
			...
			if (event.type == ALLEGRO_EVENT_KEY_DOWN && (snake1p.GetDir().x != -1 || body_length1 == 1)) {
