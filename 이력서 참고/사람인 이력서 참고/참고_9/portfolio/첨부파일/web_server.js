...
/////////////// New Player HTML ///////////////
function new_player_code (room_member, images) {
    var code = "";
    ...
    for (var key in room_member) {
        ...
        code += "<div id='player'>";
        code += "<img id='" + id + "_img' src='/upload/" + images[key] + "' width='100' height='100'/> <br/>";
...
///////////////////////////////////////////////
racing.on('connection', function(socket) {
    socket.on("chat", function(data) {
        racing.emit('chat', {id: data.id, text: data.text});
    });
});
...
//////////////////// Room /////////////////////
    socket.on("joinRoom", function(data) {
        ...
        rooms[data.room].images[temp_id] = data.img;
        ...
        connection.query('SELECT * from game_room WHERE room_num = ' + data.room + ';', function (error, results, fields) {
            ...
            if (results.length == 0) {
                ...
                socket.emit("wrong_access");
            }
            else {
                if (results[0]["playStatus"] == false) {
                    system.to(data.room).emit('new_player', new_player_code(rooms[data.room].room_member, rooms[data.room].images));
                } else {
        ...
        socket.on('disconnect', function () {
            ...
            delete rooms[data.room].images[temp_id];
            ...
            if (rooms[data.room].user_num == 0) {
                connection.query('DELETE FROM game_room WHERE room_num = ' + data.room + ';', function (error, results, fields) {
                    if (error) {
                        console.log(error);
                    }
                });
